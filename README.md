...
The site can load clients, projects and leadership from Supabase, with browser storage retained as a fallback. The project uses the public `assets` Storage bucket and the client configuration in `supabase/config.js`. Run the complete `supabase/policies.sql` script in the Supabase SQL Editor after creating an admin user in Supabase Auth. The script safely replaces existing policies, including the authenticated Storage INSERT policy required for project image uploads. The dashboard accepts the Supabase user's email and password; the legacy local login remains available for local-only testing, but remote writes require an authenticated Supabase session.
